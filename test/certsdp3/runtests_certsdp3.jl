using Test
using CertSDP

for name in names(CertSDP; all=true)
    text = String(name)
    (occursin("#", text) || startswith(text, "_")) && continue
    name in (:CertSDP, :eval, :include) && continue
    isdefined(@__MODULE__, name) && continue
    @eval const $(name) = getproperty(CertSDP, $(QuoteNode(name)))
end

    include("helpers.jl")

@testset "CertSDP 3.0 focused gates" begin
    include("kernel_trust_boundary.jl")
    include("adapter_untrusted_metadata_rejection.jl")
    include("exactify_candidates_must_replay.jl")
    include("schema_strict.jl")
    include("schema_fuzz_mutations.jl")
    include("sparse_ir.jl")
    include("psd_low_rank_factor.jl")
    include("psd_chordal_completion.jl")
    include("psd_planner_policy.jl")
    include("chordal_psd_certificate.jl")
    include("no_densification_budget.jl")
    include("proof_dag_roundtrip.jl")
    include("proof_dag_tamper.jl")
    include("test_no_hash_only_proof_checkers.jl")
    include("diagnostics_report.jl")
    include("cli_replay_explain.jl")
    include("block_native_incidence.jl")
    include("block_native_algebraic_certificate.jl")
    include("primal_dual_optimality.jl")
    include("farkas_certificate.jl")
    include("test_primal_dual_no_lhs_rhs_fallback.jl")
    include("test_primal_dual_affine_operator_replay.jl")
    include("test_farkas_no_lhs_rhs_fallback.jl")
    include("test_farkas_problem_data_replay.jl")
    include("objective_bound_certificate.jl")
    include("sparse_sos_certificate.jl")
    include("localizing_matrix_replay.jl")
    include("putinar_certificate.jl")
    include("tssos_importer.jl")
    include("tssos_tamper_rejection.jl")
    include("nctssos_importer.jl")
    include("nc_rewrite_witness.jl")
    include("quantum_bound_certificate.jl")
    include("npa_certificate_replay.jl")
    include("test_npa_every_moment_entry_replay.jl")
    include("test_npa_objective_from_verified_moments.jl")
    include("test_npa_missing_entry_node_rejects.jl")
    include("test_npa_wrong_entry_witness_rejects.jl")
    include("test_quantum_relation_semantics.jl")
    include("field_layer.jl")
    include("test_cubic_field_sign_replay.jl")
    include("test_cubic_field_sign_tamper_rejection.jl")
    include("cli_product_surface.jl")
    include("validation_cli_surface.jl")
    include("paper_bundle.jl")
    include("hash_stability.jl")
    include("backward_compatibility.jl")
    include("migration_v1_v2_to_v3.jl")
    include("algebraic_backend_interface.jl")
    include("msolve_fixture_backend.jl")
    include("backend_failure_semantics.jl")
    include("sdpa_sparse_adapter.jl")
    include("symmetry_reduction.jl")
    include("test_symmetry_group_closure_semantics.jl")
    include("test_symmetry_projector_idempotence.jl")
    include("test_symmetry_projector_orthogonality.jl")
    include("test_symmetry_projector_completeness.jl")
    include("test_symmetry_block_transform_reconstruction.jl")
    include("test_symmetry_psd_transfer_theorem.jl")
    include("mutation_corpus.jl")
    include("test_release_audit.jl")
    include("test_qa_determinism.jl")
    include("test_release_audit_strict_full.jl")
    include("test_gate_specific_audit.jl")
    include("test_dag_checker_replay.jl")
    include("test_sos_gram_expansion.jl")
    include("test_tssos_external_import.jl")
    include("test_nctssos_external_import.jl")
    include("test_cli_subprocess.jl")
    include("test_bundle_verify.jl")
    include("test_bundle_full_offline_verify.jl")
    include("test_bundle_non_placeholder_problem.jl")
    include("test_bundle_source_artifact_tamper.jl")
    include("test_bundle_object_store_tamper.jl")
    include("test_bundle_theorem_statement_tamper.jl")
    include("test_bundle_extra_untracked_proof_file_rejects.jl")
    include("test_fixture_authenticity.jl")
    include("test_true_external_raw_fixture_policy.jl")
    include("test_true_external_raw_import_replay.jl")
    include("test_true_external_raw_tamper_rejection.jl")
    include("test_ci_workflow_presence.jl")
    include("test_pass10_no_active_score_caps.jl")
    include("test_pass10_audit_report_evidence.jl")
end
