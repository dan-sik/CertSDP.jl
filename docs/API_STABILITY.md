...
CertSDP.jl keeps a deliberately small public surface.
...
| JSON problem/certificate data | Stable read/write compatibility through public entry points; legacy schema files remain readable. | Archived certificates, replay bundles, and paper artifacts. |
| CLI replay path | Public release workflow for `verify --strict`, `bundle`, and `replay`. | Independent reproduction and artifact checks. |
| Compiler extension layer | Internal unless promoted below. | Exactification, external-adapter, number-field SOS, NC, and reviewer-artifact development. |
| Internals | Not stable unless promoted here. | CertSDP implementation and tests only. |

## Stable Public API

Only these names are covered by the public compatibility contract:
...
- `read_problem(path)`: read CertSDP problem JSON, legacy prototype LMI JSON, or SDPA
  sparse files with `.dat-s`, `.dats`, or `.sdpa` extensions.
- `write_problem(path, problem)`: write canonical CertSDP problem JSON, or SDPA
  sparse when the path has an SDPA extension.
- `read_certificate(path)`: read CertSDP LMI/SOS certificates and legacy prototype
  LMI/SOS certificates.
- `write_certificate(path, certificate_or_result)`: write canonical CertSDP
  certificate JSON.
...
Patch releases may add keyword arguments and accept more
input variants, but they must not remove these names, change their core
meaning, or stop accepting valid JSON written by this release.

## Documented Experimental API

These names are documented for early expert use, but they are not yet part of
the stable compatibility promise and may require the `CertSDP.` namespace:
...
accept the stable JSON schema and the JSON emitted by earlier prototype
releases.
Use `write_problem` and `write_certificate` to re-emit canonical current JSON.
...
- `AlgebraicSOSGramProblem`, `AlgebraicSOSGramCertificate`,
  ...
  paper-artifact helper structs/functions. They are compiler implementations,
  but not yet a stable downstream extension API.

Internal APIs may change within the development series without a migration
